...
using namespace std;
    ...
    void openmp_test(int d, long double *x_test_set, long double *y_test_set, int n_size, double *weights, bool show){
      int i, j;
      double acum, y_hat;

        #pragma omp parallel for reduction(+:acum) num_threads(8) shared(x_test_set, weights, n_size, d) private(i,j)
        for (i = 0; i < n_size; i++) {
          for (j = 0; j < (d+1); j++) {
  ...
  int main (int argc, char* argv[]) {
      ...
      if (errors != 0){
        ...
        for (int i = 0; i < 10; i++) {
          t.start();
          openmp_test(d, x_test_set, y_test_set,  n_size, weights, flag);
          ms += t.stop();
        }

        cout << "OpenMP avg time = " << (ms/10) << " ms" << endl;
      }
